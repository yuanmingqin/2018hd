...
class app_captcha
{
    ...
    var $themes_jpg = array(
        1 => array('new_captcha_bg.jpg', 255, 255, 255),
    ...
    {
        $recorded = isset($_COOKIE[$this->session_word]) ? $_COOKIE[$this->session_word] : '';
        
	    if($recorded) {
			$sess = C::t("sessions_data")->fetch_by_sesskey($recorded);
			if($sess && $sess['expiry']>=time()) {
				$data = array(
					'expiry' => time()+10*60,
				);
				$condition = array(
				...
				C::t("sessions_data")->update($data, $condition);
				
				$given = $this->encrypts_word(strtoupper($word));
	        	return (preg_match("/$given/", base64_decode($sess['data'])));
			}
		}
    ...
    function generate_image($word = false)
    {
        ...
        else
        {
            ...
            $textcolor = imagecolorallocate($img_org, 118, 91, 51);
            //imagestring($img_org, 5, $x, $y, $word, $clr);
            //imagestring($img_org, 5, $x, $y, $word, $textcolor);
			for($i=0,$count=strlen($word);$i<$count;$i++){
				$startx = 80+30*$i;		
				imagettftext($img_org, 25, rand(-20,1),  $startx, rand(28, 36), $textcolor,"SIMKAI.TTF", substr($word,$i,1));
			}
    ...
    {
	    if(!$_COOKIE[$this->session_word]) {
			$recorded = md5(getglobal('clientip')."_captcha_".time());
			mySetCookie($this->session_word, $recorded);
		}
		else{
			$recorded = $_COOKIE[$this->session_word];
		}
    	
	    if($recorded) {
	    	$expiry = time()+10*60;
	    	$data = base64_encode($this->encrypts_word($word));
			$sess = C::t("sessions_data")->replace($recorded, $expiry, $data);
		}
		else{
			exit;
		}
